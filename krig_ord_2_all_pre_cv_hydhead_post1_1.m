...
text(xpos+1,ypos-15,sprintf('Range=%.2f km',range),'fontsize',16)
...
%   kv(IX)=sillv-(;
%transform data
%     zi = ((lamda.*zi)+1).^(1./lamda);
%     s2zi = ((lamda.*s2zi)+1).^(1./lamda);
end
...
figure(9)
set(gca,'FontSize',16)
contour(xii(1,:)/1000,yii(:,1)/1000,s2zi,'ShowText','on','LevelStep',10); 
% axis image; axis xy
