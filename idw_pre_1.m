...
% [~,~,well]=xlsread('C:\onedrive\phd_thesis\JOH_p2\dtw_ts_co_tradp_full.xls','hyd_head1');
[~,~,well]=xlsread('C:\Users\WEM\OneDrive\phd_papers\Acta_geophysica\JOH_p2\gis\extract_poits.xlsx');
...
[X1,map1,R1]=geotiffread('C:\Users\WEM\OneDrive\phd_papers\Acta_geophysica\JOH_p2\gis\topo_idex1.tif');
[X,map,R]=geotiffread('C:\Users\WEM\OneDrive\phd_papers\Acta_geophysica\JOH_p2\gis\maps\bound_90_4.tif');
% [~,~,ints]=xlsread('C:\phd_thesis\JOH_p2\gis\trans_xy2.xlsx','trans_xy2');
bnd = shaperead('C:\Users\WEM\OneDrive\phd_papers\Acta_geophysica\JOH_p2\gis\pol_grid\pol_grid.shp')
...
%     t_ind=[20,22,27,29];
     ...
     Fidw=IDW([mat1(:,1),mat1(:,2)],mat1(:,4),[xii(:),yii(:)]);
     vidw=IDW([mat1(:,1),mat1(:,2)],mat1(:,4),[x(k),y(k)]);
     zobs(k,1)=wlt(k,1);
     zpred(k,1)=vidw; 
     k
...
[rsq(i),rmse1(i)]=rsquare(zobs,zpred);
end
Zv=nan(length(xii(:,1)),length(xii(1,:)));

TP=reshape(Fidw,size(xii,1),size(xii,2));
...
figure(2)
contourf(xii, yii, reshape(Fidw,size(xii,1),size(xii,2)))
