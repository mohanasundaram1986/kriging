...
[X1,map1,R1]=geotiffread('C:\Users\WEM\OneDrive\phd_papers\Acta_geophysica\JOH_p2\gis\dist_stream_ras.tif');
...
dem_wells=well(2:end,252);
dem_wells=cell2mat(dem_wells);
dem_wells(isnan(dem_wells))=0.1;
%create meshgrid
minx=R1(1,1);
maxx=R1(2,1);
miny=R1(1,2);
...
demgrid=X1(ind1);
% demgrid(demgrid==NaN)=1.0;
% demgrid(demgrid<=0)=1.0;
...
%    zobs(k,1)=zdemv; 
   ...
   xlabel('Distance from streams (m)')
...
text(xpos+1,ypos-2.5,sprintf('Sill=%.2f m^2',sill),'fontsize',16)
text(xpos+1,ypos-8.5,sprintf('Range=%.2f km',range),'fontsize',16)
